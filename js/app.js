    ...
    newTodoInput.addEventListener('keyup', function addTodoController(event){
        if (event.keyCode === 13) { // AND presses the "ENTER" key
          ...
          todos.taskList.push(task);
					...
					if (document.querySelector('span.todo-count').textContent === 1) {
						else {(todos.taskList.length + "Tasks left"); }
					}

					document.querySelector('span.todo-count').textContent = (todos.taskList.length + "Tasks Left");
